...
def generate_motions_recursive(num_agents,cur_agent):
    ...
    #joint_state_motions = list(combinations_with_replacement(directions, num_agents))
...
def push_node_focal(focal_list, node):
    heapq.heappush(focal_list, (node['d'], node['g_val'] + node['h_val'], node['h_val'], node['loc'], node))

def pop_node(open_list):
    _, _, _, curr = heapq.heappop(open_list)
    return curr

def pop_node_focal(focal_list):
...
"""def not_all_in_obstacles(map, locs):
    for loc in locs:
        if not map[loc[0]][loc[1]]:
            return False
    return True"""

def a_star(my_map, start_loc, goal_loc, h_values, agent, constraints, paths_other_robots):
    ...
    def calculate_d_values():
        # calculates the d heuristic for the map
        paths = copy.deepcopy(paths_other_robots)
        if paths == []:
            return {}
        else:
            paths.pop(agent)
    ...
    open_list = []
    focal_list = [] # new line: define a focal list that sorts based on d value
    closed_list = dict()
    earliest_goal_timestep = 0
    h_value = h_values[start_loc]
    root = {'loc': start_loc, 'g_val': 0, 'h_val': h_value, 'd': 0,'parent': None, 'timestep': 0} #edit: add 'd' value
    push_node(open_list, root)
    push_node_focal(focal_list, root) # new line: push to focal_list
    closed_list[(root['loc'], root['timestep'])] = root
    fmin = root['g_val'] + root['h_val'] # new line -> instantiate fmin
    print(open_list)
    print(focal_list)

    while len(focal_list) > 0: #edit: open_list -> focal_list
        curr = pop_node_focal(focal_list) #edit: open_list -> focal_list
        open_list = [node for node in open_list if node[-1] != curr]
        '''for i, lst in enumerate(open_list):
            if lst[-1] == curr:
                open_list.pop(i)'''
        #open_list.remove(curr) #new line -> remove node from open_list
        #############################
        ...
        for dir in range(5):
            ...
            d_val = d_values[(child_loc, curr['timestep'] + 1)] if (child_loc, curr['timestep'] + 1) in d_values else 0 #new line -> searches d value for child node
                
            child = {'loc': child_loc,
                    'g_val': curr['g_val'] + 1,
                    'h_val': h_values[child_loc],
                    'd': d_val, #edit -> d value added to child node
                    'parent': curr,
            ...
            if (child['loc'], child['timestep']) in closed_list:
                ...
                if compare_nodes(child, existing_node):
                    ...
                    if child['g_val'] + child['h_val'] < w*fmin: #new line -> condition to put node in focal list
                        push_node_focal(focal_list, child)
            else:
                closed_list[(child['loc'], child['timestep'])] = child
                push_node(open_list, child)
                if child['g_val'] + child['h_val'] < w*fmin: #new line -> condition to put node in focal list
        ...
        if open_list[0][0] > fmin: # update focal list
            fmin = open_list[0][0]
            focal_list = []
            #focal_list = [push_node_focal(focal_list, lst[-1]) for lst in open_list if lst[0] < w*fmin]
            for lst in open_list:
                if lst[0] < w*fmin:
                    push_node_focal(focal_list, lst[-1])
