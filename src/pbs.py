...
class PBSSolver(object):
    ...
    def update_plan(self,node,i):
        ...
        lp_agents = get_lower_priority_agents(node['priority_pairs'], i)

        for j in lp_agents:
            
            if j == i or collide_with_higher_priority_agents(node, j):
                constraints = []
                hp_agents = get_higher_priority_agents(node['priority_pairs'], j)
                print(hp_agents)
                for k in hp_agents:
                    if k == j:
                        continue
                    for t in range(len(node['paths'][k])):
                        constraints.append({'agent': j, 'loc': [node['paths'][k][t]], 'timestep': t})
                    for t in range(len(node['paths'][k]) - 1):
                        constraints.append({'agent': j, 'loc': [node['paths'][k][t+1], node['paths'][k][t]], 'timestep': t+1})
                    constraints.append({'agent': j, 'loc': node['paths'][k][-1], 'timestep': len(node['paths'][k]) - 1, 'type': 'inf'})
    ...
    def find_solution(self):
        ...
        root = {'cost': 0,
                'priority_pairs': [],
                'paths': [[] for k in range(self.num_of_agents)],
                'collisions': []}
